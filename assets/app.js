...
function propertyCard(property) {
  ...
  return `<article class="property-card ${hasImages ? '' : 'property-card-placeholder'}">
    ...
    <div class="property-body">
      <div class="property-meta"><span>${esc(propertyTypeLabel(property.property_type))}</span>${refVisible(property) ? `<span>Ref. ${esc(refVisible(property))}</span>` : ''}</div>
...
/* 115 de los 200 inmuebles no tienen número de referencia en el origen, así que
   se derivó del slug. Mostrarlo daría "Ref. fantastico-chalet-en-venta-con-...".
   Sólo se enseña cuando es una referencia real de Noguera. */
...
function adminPropertiesView() {
  ...
  return adminLayout(`<div class="admin-heading"><div><span class="admin-eyebrow">Cartera</span><h1>Inmuebles</h1><p>Edita la ficha, cambia la disponibilidad y controla qué aparece en la web.</p></div><a href="/admin/inmuebles/nuevo" data-link class="btn btn-accent">${icons.plus} Nuevo inmueble</a></div><section class="admin-card"><form class="admin-filter-bar" data-form="admin-filter"><div class="admin-search">${icons.search}<input class="input" name="q" value="${attr(params.get('q')||'')}" placeholder="Buscar por título, referencia o zona" aria-label="Buscar por título, referencia o zona"></div><select class="select" name="status"><option value="">Todos los estados</option><option value="published" ${status==='published'?'selected':''}>Publicados</option><option value="draft" ${status==='draft'?'selected':''}>Borradores</option><option value="archived" ${status==='archived'?'selected':''}>Archivados</option></select><select class="select" name="availability"><option value="">Toda disponibilidad</option><option value="available" ${availability==='available'?'selected':''}>Disponibles</option><option value="reserved" ${availability==='reserved'?'selected':''}>Reservados</option><option value="sold" ${availability==='sold'?'selected':''}>Vendidos</option><option value="rented" ${availability==='rented'?'selected':''}>Alquilados</option></select><button class="btn btn-primary btn-sm" type="submit">Filtrar</button><a href="/admin/inmuebles" data-link class="btn btn-ghost btn-sm">Limpiar</a><span class="catalog-count">${items.length} resultado${items.length===1?'':'s'}</span></form>${adminPropertiesTable(items)}</section>`);
}
...
/* Las plantillas escriben <label>Texto</label><input ...> como hermanos, sin
   for/id, así que la etiqueta no queda asociada: al pulsarla no se enfoca el
   campo y los lectores de pantalla anuncian el control sin nombre. Se asocian
   aquí en lugar de en las ~50 plantillas para que valga también para los
   campos que se añadan después. */
let fieldSeq = 0;
