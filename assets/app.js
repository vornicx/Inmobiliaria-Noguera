...
function publicFooter() {
  return `<footer class="footer">
    ...
    <div class="container footer-grid">
      <div class="footer-brand-col"><a href="/" data-link>${logo({ light: true })}</a><p class="footer-intro">Inmuebles y acompañamiento inmobiliario en Écija y comarca desde una oficina a la que puedes venir, llamar y volver.</p><div class="footer-social"><a href="${attr(BUSINESS.instagram)}" target="_blank" rel="noopener">Instagram</a><a href="${attr(BUSINESS.facebook)}" target="_blank" rel="noopener">Facebook</a></div></div>
...
function homeView() {
  ...
  return publicLayout(`
    <section class="hero v5-hero">
      ...
      <div class="container hero-inner"><div class="hero-copy"><span class="hero-kicker"><i></i> Inmobiliaria en Écija desde 2007</span><h1 class="hero-title">Tu próxima vivienda, con una búsqueda más clara.</h1><p class="hero-sub">Explora una cartera real y habla con un equipo local que te acompaña desde la primera visita hasta la firma.</p><div class="hero-actions"><a href="/inmuebles" data-link class="btn btn-light">Ver inmuebles ${icons.arrow}</a><a href="/vender-vivienda-ecija" data-link class="hero-text-link">Quiero vender ${icons.arrow}</a></div></div></div>
    </section>
    <section class="home-search-wrap" aria-label="Buscador de inmuebles"><div class="search-shell"><div class="search-tabs" role="tablist" aria-label="Tipo de búsqueda"><button class="search-tab active" data-search-tab="venta" role="tab">Comprar</button><button class="search-tab" data-search-tab="alquiler" role="tab">Alquilar</button><button class="search-tab" data-search-tab="vacacional" role="tab">Vacacional</button></div><form class="search-bar" data-form="hero-search"><input type="hidden" name="operation" value="venta"><div class="search-field search-field-main"><label>¿Dónde buscas?</label><input name="q" placeholder="Zona, localidad o referencia"></div><div class="search-field"><label>Tipo</label><select name="type"><option value="">Todos los inmuebles</option>${PROPERTY_TYPES.map(([v,l]) => `<option value="${v}">${l}</option>`).join('')}</select></div><div class="search-field"><label>Precio máximo</label><select name="max"><option value="">Sin límite</option><option value="100000">100.000 €</option><option value="150000">150.000 €</option><option value="250000">250.000 €</option><option value="400000">400.000 €</option><option value="700000">700.000 €</option></select></div><button class="btn search-submit" type="submit">${icons.search}<span>Buscar</span></button></form></div></section>
    <section class="trust-strip"><div class="container trust-strip-grid"><div><strong>Desde 2007</strong><span>Experiencia local en Écija</span></div><div><strong>Oficina física</strong><span>${esc(BUSINESS.address)}</span></div><div><strong>Atención directa</strong><span>Un equipo durante toda la operación</span></div><div><strong>Cartera gestionable</strong><span>Disponibilidad y precios actualizables</span></div></div></section>
    <section class="section featured-section" id="seleccion"><div class="container"><div class="section-heading section-heading-row"><div><span class="eyebrow">Selección Noguera</span><h2 class="h2">Viviendas y oportunidades destacadas.</h2></div><div><p class="lead">Una selección real de la cartera para empezar a comparar sin perderte entre anuncios.</p><a href="/inmuebles" data-link class="editorial-link">Explorar toda la cartera ${icons.arrow}</a></div></div>${propertiesGrid(featured, 'home-featured-grid')}</div></section>
    <section class="section type-discovery"><div class="container"><div class="section-heading section-heading-row"><div><span class="eyebrow">Busca a tu manera</span><h2 class="h2">Empieza por el tipo de propiedad.</h2></div><p class="lead">Accede directamente a las categorías más consultadas.</p></div><div class="type-discovery-grid"><a href="/inmuebles?type=piso" data-link class="type-tile type-tile-large"><img src="${attr(safeImage(typePicks[0]?.images?.[0]))}" ${imageErrorAttr(typePicks[0])} alt="Pisos y áticos de la cartera Noguera" loading="lazy" decoding="async"><span><small>Pisos y áticos</small><strong>Vivir en la ciudad</strong><em>Ver propiedades ${icons.arrow}</em></span></a><a href="/inmuebles?type=casa" data-link class="type-tile"><img src="${attr(safeImage(typePicks[1]?.images?.[0]))}" ${imageErrorAttr(typePicks[1])} alt="Casas y chalets de la cartera Noguera" loading="lazy" decoding="async"><span><small>Casas y chalets</small><strong>Más espacio, más posibilidades</strong><em>Ver propiedades ${icons.arrow}</em></span></a><a href="/inmuebles?type=nave" data-link class="type-tile"><img src="${attr(safeImage(typePicks[2]?.images?.[0]))}" ${imageErrorAttr(typePicks[2])} alt="Locales, naves y terrenos de la cartera Noguera" loading="lazy" decoding="async"><span><small>Negocio e inversión</small><strong>Locales, naves y suelo</strong><em>Ver oportunidades ${icons.arrow}</em></span></a></div></div></section>
